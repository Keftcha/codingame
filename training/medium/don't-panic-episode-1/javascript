...
for (var i = 0; i < nbElevators; i++) {
    var inputs = readline().split(' ');
    var elevatorFloor = parseInt(inputs[0]); // floor on which this elevator is
found
    var elevatorPos = parseInt(inputs[1]); // position of the elevator on its
floor
...
while (true) {
    ...
    var clonePos = parseInt(inputs[1]); // position of the leading clone on its
floor
    var direction = inputs[2]; // direction of the leading clone: LEFT or RIGHT

    var valeur = dico[cloneFloor];
    if (valeur === undefined) {
        var valeur = exitPos
    }

    if (Number(clonePos) < Number(valeur) && direction === "LEFT") {
        instruction = "BLOCK"
    } else if (Number(clonePos) > Number(valeur) && direction === "RIGHT") {
        instruction = "BLOCK"
    } else {
        instruction = "WAIT"
    }

    print(instruction); // action: WAIT or BLOCK
    idx += 1
}
